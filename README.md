...
This satisfies the CRDT laws (commutativity, associativity, idempotency). Commutativity and idempotency are derived algebraically from confluence (Corollary 5.2 of the formal theory). Associativity is verified by the property test suite (`prop_merge_associative`) rather than by a closed-form algebraic proof.
...
See `FORMAL_THEORY.md` (and its rendered PDF) for the complete mathematical treatment, including:
...
- Merge semilattice laws: commutativity and idempotency proven algebraically; associativity verified by property tests (`prop_merge_associative`)
